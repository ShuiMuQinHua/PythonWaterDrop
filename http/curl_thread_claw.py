...
class ThreadCrawl(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            try:
                req = urllib.request.Request(url=self.url, data=data.encode('utf-8'))
                res = urllib.request.urlopen(req)

            except urllib.request.HTTPError as e:
                raise e.reason
            #将 JSON 对象转换为 Python 数据
            py_data = json.loads(json.dumps(res.read().decode('utf-8')))  #.read().decode('utf-8')
            res.close()
            item['first'] = 'false'
            item['pn'] = item['pn'] + 1
